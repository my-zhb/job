...
<header class="joe_header">
        ...
        <div class="joe_container">
            ...
            <a title="<?php $this->options->title(); ?>" class="joe_header__above-logo" href="<?php $this->options->siteUrl(); ?>">
                <?php if ($_SERVER['HTTP_HOST'] == 'zhinianblog.com' or $_SERVER['HTTP_HOST'] == 'www.zhinianblog.com') : ?>
                    <img class="lazyload" src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-src="<?php $this->options->JLogo() ?>" alt="<?php $this->options->title(); ?>" style="width: 150px;"/>
                <?php else : ?>
                ...
                if ($custom_text) {
                    ...
                    if (count($custom_arr) > 0) {
                        for ($i = 0; $i < count($custom_arr); $i++) {
                            ...
                            // add by zhinianblog start
							$isPage = explode("||", $custom_arr[$i])[2];
							if($isPage) {
								$customPage[] = array("title" => trim($title), "url" => trim($url));
							}
							else {
            ...
            <nav class="joe_header__above-nav">
                ...
                <?php if (count($pages->stack) <= $this->options->JNavMaxNum) : ?>
                    ...
                    <?php endforeach; ?>
                    <!-- add by zhinianblog start -->
                    <?php if (sizeof($customPage) > 0) : ?>
                        <?php foreach ($customPage as $item) : ?>
                            <a target="_blank" rel="noopener noreferrer" class="item" href="<?php echo $item['url'] ?>" title="<?php echo $item['title'] ?>"><?php echo $item['title'] ?></a>
                        <?php endforeach; ?>
                    <?php endif; ?>
                    <!-- add by zhinianblog end -->
                <?php else : ?>
    ...
    <div class="joe_header__slideout">
        ...
        <ul class="joe_header__slideout-count">
            ...
            <li class="item" style="<?php if(Helper::options()->TodayArchive4Phone !== 'on') echo 'display:none;'  ?>">
                <svg class="icon" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" width="15" height="15">
                    <path d="M606.227 985.923H164.75c-69.715 0-126.404-56.722-126.404-126.442V126.477C38.346 56.755 95.04 0 164.75 0h619.275c69.715 0 126.549 56.755 126.549 126.477v503.925c0 18.216-14.814 32.997-33.07 32.997-18.183 0-32.925-14.78-32.925-32.997V126.477c0-33.355-27.2-60.488-60.554-60.488H164.75c-33.353 0-60.41 27.133-60.41 60.488v733.004c0 33.353 27.057 60.441 60.41 60.441h441.477c18.183 0 32.925 14.787 32.925 33.004 0 18.211-14.742 32.997-32.925 32.997zm0 0" />
                    <path d="M657.62 322.056H291.154c-18.183 0-32.924-14.786-32.924-33.003 0-18.21 14.74-32.998 32.924-32.998H657.62c18.256 0 33.07 14.787 33.07 32.998 0 18.217-14.814 33.003-33.07 33.003zm0 0M657.62 504.749H291.154c-18.183 0-32.924-14.78-32.924-32.993 0-18.222 14.74-32.997 32.924-32.997H657.62c18.256 0 33.07 14.775 33.07 32.997 0 18.218-14.814 32.993-33.07 32.993zm0 0M445.611 687.486H291.154c-18.183 0-32.924-14.78-32.924-33.004 0-18.21 14.74-32.991 32.924-32.991h154.457c18.184 0 32.998 14.78 32.998 32.991 0 18.224-14.814 33.004-32.998 33.004zm0 0M866.482 1024c-8.447 0-16.896-3.225-23.34-9.662L577.595 748.786c-7.156-7.123-10.592-17.07-9.446-27.056l8.733-77.728c1.788-15.321 13.885-27.378 29.2-29.06l77.45-8.52c10.443-.965 19.9 2.433 26.905 9.449l265.558 265.551c12.875 12.877 12.875 33.784 0 46.666l-86.184 86.25c-6.438 6.437-14.887 9.662-23.33 9.662zm-231.05-310.646l231.05 231.018 39.575-39.62-231.043-231.05-35.505 3.938-4.076 35.714zm0 0" />
                </svg>
                <span>今日撰写 <strong><?php echo getNumPosts(1); ?></strong> 篇文章</span>
            </li>
        </ul>
