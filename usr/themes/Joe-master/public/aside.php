<aside class="joe_aside">
    ...
    <?php if ($this->options->JADContent) : ?>
        <?php 
            $JIndex_Adaside = [];
			$JIndex_Adaside_text = $this->options->JADContent;
            if ($JIndex_Adaside_text) {
                $JIndex_Adaside_arr = explode("\r\n", $JIndex_Adaside_text);
                if (count($JIndex_Adaside_arr) > 0) {
                    for ($i = 0; $i < count($JIndex_Adaside_arr); $i++) {
                        $adImgAside = explode("||", $JIndex_Adaside_arr[$i])[0];
                        $adUrlAside = explode("||", $JIndex_Adaside_arr[$i])[1];
                        $JIndex_Adaside[] = array("adUrl" => trim($adUrlAside), "adImg" => trim($adImgAside));
                    };
                }
            }
        ?>
	    <?php if (sizeof($JIndex_Adaside) > 0) : ?>
		    <?php foreach ($JIndex_Adaside as $item) : ?>
		        <a class="joe_aside__item advert" target="_blank" rel="noopener noreferrer nofollow" href="<?php echo $item['adUrl'] ?>" title="广告">
                    <img class="lazyload" width="100%" src="<?php _getLazyload() ?>" data-src="<?php echo $item['adImg'] ?>" alt="广告" />
