...
?>
    ...
    <link rel="stylesheet" href="<?php Helper::options()->themeUrl('typecho/config/css/back.custom.css') ?>">
    <style>
        .joe_config .joe_config__custom {
            margin-left: 15px;
        }
        
    </style>
    <script src="<?php Helper::options()->themeUrl('typecho/config/js/joe.config.min.js') ?>"></script>
    <script src="<?php Helper::options()->themeUrl('assets/js/custom.js?v=20210602') ?>"></script>
    <div class="joe_config">
        <div>
            <div class="joe_config__aside">
                <div class="logo">Joe <?php echo _getVersion() ?>
                    <p style="font-size: 5px;" id="zhinianblog_version">V2.0</p>
                </div>
                <ul class="tabs">
                    ...
                    <li class="item" data-current="joe_navigation">个人导航</li>
        ...
        <div class="joe_config__dingyue joe_config__custom" style="display: none;"><iframe src="https://dy.zhinianblog.com/dy/dingyue/DingYue005/init"  width="100%" height="750px" scrolling="auto"></iframe></div>
        <div class="joe_config__caiji joe_config__custom" style="display: none;"><iframe src="https://dy.zhinianblog.com/system/caiji/CaiJi001/init"  width="100%" height="750px" scrolling="auto"></iframe></div>
        <div class="joe_config__link joe_config__custom" style="display: none;"><iframe src="https://dy.zhinianblog.com/frdlk/friends/AutoFriendsSumit002/init" width="100%" height="750px" scrolling="auto"></iframe></div>
        <div class="joe_config__themeUpdate">
            <h5>请开启插件后使用，插件下载地址：<a href="https://zhinianblog.com/archives/993/">https://zhinianblog.com/archives/993/</a></h5>
            <p style="margin-left: 20px;margin-top: 20px;"><label>&nbsp;&nbsp;&nbsp;授权码：</label><input id="AutoUpdateTheme_cardId" type="text" style="margin-left: 12px;"/></p>
            <p style="margin-left: 20px;margin-top: 20px;"><label>主题名称：</label><input id="AutoUpdateTheme_themeName" disabled type="text" style="margin-left: 10px;" value="<?php 
                    $themeUrl = Helper::options()->themeUrl;
                    $oldThemeName = substr($themeUrl, strrpos($themeUrl, '/') + 1);
                    echo $oldThemeName; 
                ?>"/>
            </p>
            <p style="margin-left: 20px;margin-top: 20px;">
                <label>更新版本：</label>
                <select id="AutoUpdateTheme_downLoadUrl" style="margin-left: 8px;width: 186px;height: 32px">
                    <option value="http://download.zhinianblog.com/themes/Joe-master_zhinian.zip">执念博客同款</option>
                    <option value="http://download.zhinianblog.com/themes/Joe-master.zip">Joe官方主题</option>
                </select>
            </p>
            <button id="AutoUpdateTheme_submit" style="margin-left: 100px;margin-top: 20px; width: 120px;" class="btn primary">开始更新</button>
        </div>
    <?php
    ...
    $JCustomNavs = new Typecho_Widget_Helper_Form_Element_Textarea(
        ...
        '导航栏自定义链接（非必填）',
        '介绍：用于自定义导航栏链接 <br />
         格式：跳转文字 || 跳转链接 || 同页面格式（中间使用两个竖杠分隔）<br />
    ...
    $JIndex_Ad = new Typecho_Widget_Helper_Form_Element_Textarea(
        ...
        '首页大屏广告',
        '介绍：请务必填写正确的格式 <br />
         格式：广告图片 || 广告链接 || 到期日(可不填写)（中间使用两个竖杠分隔）<br />
         例如：https://puui.qpic.cn/media_img/lena/PICykqaoi_580_1680/0 || https://baidu.com || 10-10'
    );
    ...
    $JSensitiveWords = new Typecho_Widget_Helper_Form_Element_Textarea(
        ...
        '你妈死了,傻逼,操你妈,射你妈一脸',
        '评论敏感词（非必填）',
        '介绍：用于设置评论敏感词汇，如果用户评论包含这些词汇，将会禁止评论并直接骂回去 <br />
